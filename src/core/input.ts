export type Button =
  | 'up' | 'down' | 'left' | 'right'
  | 'shoot' | 'bomb' | 'focus' | 'hyper' | 'pause' | 'confirm' | 'back';
...
// Keyboard state with an official per-frame injection hook (Input.inject) —
// the AutoplayAI drives the game through exactly the same path as a human.
export class Input {
